...
const app = workflow.compile({checkpointer: memory , interruptBefore:["agent"]});
...
const startConversation = async (): Promise<void> => {
    rl.question("You: ", async (input: string) => {
      ...
      try {
        // First invoke with user input
        const initialResponse = await app.invoke({
        ...
        // Case 2: Tool approval needed
        /* if (lastMessage.tool_calls?.length) {
          const toolName = lastMessage.tool_calls[0].name;
          console.log(`Agent: Can I have access to this tool: ${toolName}? (yes/no)`);
          
          rl.question("You: ", async (approval: string) => {
            if (approval.toLowerCase() === "yes") {
              try {
                // Proceed with tool execution
                const toolResponse = await app.invoke(null, config);
                const toolResult = toolResponse.messages[toolResponse.messages.length - 1].content;
                console.log('Agent:', toolResult);
              } catch (toolError) {
                console.error('Tool execution failed:', toolError);
              }
            } else {
              console.log('Agent: Tool execution cancelled.');
            }
            startConversation(); // Continue conversation
          });
          return; // Important: Return here to avoid double continuation
        } */

        // case3: state update
        if (lastMessage instanceof  HumanMessage) {
          console.log(`Agent: update your state`);
          
          rl.question("state_update: ", async (update: string) => {
            await app.updateState(config , {"messages":[new HumanMessage(update)]}) // this will update the state with the reducer defined 
            await app.invoke(null, config);                                         // in our case, "updateState" will add to the BaseMessages() array !
            const res = await app.invoke(null, config); // the reason it was invoked twice becuse it will get interrupted again !!!
            console.log(res)
            const toolResult = res.messages[res.messages.length - 1].content;
            console.log('Agent:', toolResult);
